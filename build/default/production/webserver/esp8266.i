...
# 12 "webserver/esp8266.c" 2
# 62 "webserver/esp8266.c"
void esp_setUARTConfiguration(void) {
...
# 103 "webserver/esp8266.c"
void esp_getVersion(void) {
    printf("AT+GMR\r\n");
    while (!esp_uartReceive("OK") || !esp_uartReceive("ERROR")) {};
}
# 139 "webserver/esp8266.c"
void esp_upgradeFirmware(void) {
    printf("AT+CIUPDATE\r\n");
...
# 169 "webserver/esp8266.c"
void esp_echoOff(void) {
    printf("ATE0\r\n");
    while( !esp_uartReceive("OK") || !esp_uartReceive("ERROR") )
        ;
}
# 190 "webserver/esp8266.c"
void esp_echoOn(void) {
    printf("ATE1\r\n");
    do { esp_waitForOK(); } while( server.busy );
}
# 214 "webserver/esp8266.c"
        ...
        for(i=0; i<= 64; i++){
            ...
            if(tmp == 0x0A) {
                ...
                if( (strcmp(string, "OK") == 0) || (strcmp(string, "no change") == 0) || (strcmp(string, "Send OK") == 0) ) {
                    ...
                    server.wificonnected = 0;
                    server.wifigotip = 0;
                    states.ok = 0;
                    states.error = 1;
                    break;
                } else if ( strcmp(string, "WIFI CONNECTED") == 0 ) {
                    server.wificonnected = 1;
                    break;
                    ...
                    server.clientConnected = 1;
                    break;
                } else if ( strncmp(string, "+IPD" ,4) == 0 ) {
                    server.clientAvailable = 1;
                    tcpip_serverPage();
                    break;
                } else if ( strncmp(string, "+CWJAP", 6) == 0 ) {
                    do { LATAbits.LATA4 = 1; } while(0);
                } else if ( strncmp(string, "No AP", 5) == 0) {
                    server.wificonnected = 0;
                    server.wifigotip = 0;
                    break;
                    ...
                    if( strcmp((char*)server.ssid, "") == 0) {
                        ...
                        strcpy(src, string+9);
                        length = (uint8_t) strlen(string)-11;
...
void esp_adjustLeds(void) {
    ...
    if ( server.wificonnected == 1 && server.wifigotip == 1 )
